...
class LinearEmbedR(nn.Module):
    """Linear Embedding"""
    def __init__(self, dim_in=1024, dim_out=100):
        super(LinearEmbedR, self).__init__()
        self.linear1 = nn.Linear(dim_in,100)
        nn.init.kaiming_normal_(self.linear1.weight, nonlinearity='relu')  # Kaiming initialization
        #self.linear2 = nn.Linear(128, 100)
        #nn.init.kaiming_normal_(self.linear2.weight, nonlinearity='relu')  # Kaiming initialization
        #self.linear3 = nn.Linear(768, dim_out)
        #nn.init.kaiming_normal_(self.linear3.weight, nonlinearity='relu')  # Kaiming initialization
    ...
    def forward(self, x):
        ...
        #x = self.drop2(self.relu2(self.bn2(x)))
        #x = self.drop3(self.linear3(x))
        return x
