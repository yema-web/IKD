...
from models.util import Embed, ConvReg, LinearEmbed,LinearEmbedR
...
from distiller_zoo import PKT, ABLoss, FactorTransfer, KDSVD, FSP, NSTLoss,DKD,NKDLoss,WTTMLoss
...
def parse_option():
    ...
    # distillation
    parser.add_argument('--distill', type=str, default='kd', choices=['kd', 'dkd','nkd',"wttm","hint", 'attention', 'similarity',
    ...
    # WTTM distillation
    parser.add_argument('--wttm_gamma', default=0.3, type=float)
    ...
    parser.add_argument('--margin_w', type=float, default=1.0, help='weight for hinge loss1')
    parser.add_argument('--avg', type=float, default=1.0, help='weight for intra distillation loss')
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}_r:{}_a:{}_b:{}_intrat:{}_avg:{}_{}'.format(opt.model_s, opt.model_t, opt.dataset, opt.distill,
                                                                opt.gamma, opt.alpha, opt.beta, opt.intra_T,opt.avg,opt.trial)
...
def main():
    ...
    if opt.distill == 'kd':
        ...
        criterion_kd = NKDLoss(opt.kd_T,opt.beta)
    elif opt.distill == 'wttm':
        criterion_kd = WTTMLoss()
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        #print("==> training...")
        time1 = time.time()
        train_acc, train_loss,avg_logit,avg_feature = train(epoch, avg_logit,avg_feature,train_loader, module_list, criterion_list, optimizer, opt)
        time2 = time.time()
